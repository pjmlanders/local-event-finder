  ...
  if (events.length === 0) {
    return (
      <div className="py-16 text-center">
        ...
        <p className="mt-4 font-medium text-slate-600">No events found</p>
        <p className="mt-1 text-sm text-slate-500">Try adjusting your filters or expanding the radius</p>
      </div>
    )
  }
    ...
    return (
      <div className="space-y-6">
        {Array.from(grouped.entries()).map(([date, dayEvents]) => (
          <div key={date}>
            <h3 className="mb-3 flex items-center gap-3 text-sm font-semibold text-slate-600">
              <span className="h-px flex-1 bg-slate-200" />
              {formatEventDate(date)}
              <span className="rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-medium text-slate-600">{dayEvents.length}</span>
              <span className="h-px flex-1 bg-slate-200" />
            </h3>
