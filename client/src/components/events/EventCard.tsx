...
export default function EventCard({ event, compact = false, showDates = false, dateRange, showCount, featured = false }: EventCardProps) {
  ...
  if (compact) {
    return (
      <Link
        to={`/event/${event.id}`}
        className="surface-card flex items-center gap-3 p-4 transition-all hover:-translate-y-0.5 hover:shadow-md"
      >
        ...
        <div className="min-w-0 flex-1">
          <h4 className="truncate text-sm font-semibold text-slate-900">{event.name}</h4>
          <p className="truncate text-xs text-slate-500">{event.venue.name} &middot; {formatEventTime(event.startTime)}</p>
        </div>
        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 flex-shrink-0 text-slate-400" viewBox="0 0 20 20" fill="currentColor">
  ...
  if (featured) {
    return (
      <Link
        to={`/event/${event.id}`}
        className="group block overflow-hidden rounded-2xl bg-slate-900 shadow-xl transition-all hover:shadow-2xl hover:-translate-y-0.5"
      >
  ...
  return (
    <Link
      to={`/event/${event.id}`}
      className="group flex flex-col overflow-hidden surface-card transition-all hover:-translate-y-0.5 hover:shadow-md"
    >
      {/* Image */}
      ...
      <div className="flex flex-1 flex-col p-3.5">
        ...
        {/* Title */}
        <h3 className="mb-1 line-clamp-2 text-sm font-semibold leading-snug text-slate-900 group-hover:text-indigo-600">
          {event.name}
        </h3>

        {/* Venue */}
        <p className="mb-auto line-clamp-1 text-xs text-slate-500">
          {event.venue.name} &middot; {event.venue.city}, {event.venue.state}
        </p>
        ...
        <div className="mt-3 flex items-center justify-between border-t border-slate-100 pt-2.5">
          <div className="flex items-center gap-2">
            ...
            <span className={`rounded px-1.5 py-0.5 text-[10px] font-medium ${SOURCE_COLORS[event.source] ?? 'bg-slate-100 text-slate-500'}`}>
